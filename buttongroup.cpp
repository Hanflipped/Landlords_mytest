...
void ButtonGroup::initButtons()
{
    ...
    //设置按钮的大小,采用容器，使用for循环进行设置，避免程序冗余
    QVector<MyButton*> btns; //Qvector的用法
    ...
    //在当前类别里发射信号，不要处理相关动作
    connect(ui->start, &MyButton::clicked, this, &ButtonGroup::startGame); //??该函数的意义与作用是？
